...
/**
 * Waterline-owned visibility metadata used by both embedded and service mode.
 * Query execution remains the responsibility of the selected backend.
 */
final class VisibilityFilterContract
{
    ...
    public static function versionMetadata(mixed $version): array
    {
        ...
        return [
            ...
            'message' => $supported ? null : self::unsupportedVersionMessage($normalizedVersion, $supportedVersions),
        ];
    }
    ...
    public static function fromRequest(Request $request): array
    {
        ...
        foreach ([...self::STRING_FIELDS, ...self::BOOLEAN_FIELDS] as $field) {
            $filters[$field] = $request->query($field);
        }

        foreach (array_keys(self::CONTAINS_FIELDS) as $field) {
            $filters[$field] = $request->query($field);
        }
    ...
    public static function merge(array ...$filters): array
    {
        ...
        foreach ($filters as $filter) {
            ...
            foreach ([...self::STRING_FIELDS, ...self::BOOLEAN_FIELDS, ...array_keys(self::CONTAINS_FIELDS)] as $field) {
    ...
    private static function stringValue(mixed $value): ?string
    {
        ...
        return $value === '' || strlen($value) > 191 ? null : $value;
    }
    ...
    private static function normalizeVersion(mixed $version): ?int
    {
        ...
        return $version !== '' && ctype_digit($version) ? (int) $version : null;
    }
    ...
    private static function fieldDefinition(
        ...
        $definition = [
            ...
            'type' => $type,
            'input' => $type === 'boolean' ? 'boolean_select' : ($options === [] ? 'text' : 'select'),
            'operator' => $operator,
            'filterable' => true,
            'saved_view_compatible' => true,
            'order' => $order,
        ...
        if (($help = self::helpForField($field)) !== null) {
    ...
    private static function optionsForField(string $field, string $type): array
    {
        ...
        return match ($field) {
            'status' => self::options(['pending', 'running', 'waiting', 'cancelled', 'terminated', 'completed', 'failed']),
            'status_bucket' => self::options(['running', 'completed', 'failed']),
            'closed_reason' => self::options(['completed', 'failed', 'cancelled', 'terminated', 'continued']),
            'wait_kind' => self::options(['activity', 'update', 'signal', 'timer', 'condition', 'workflow-task', 'child']),
            'repair_blocked_reason' => self::repairBlockedReasonOptions(),
            'declared_entry_mode' => [['label' => 'Canonical', 'value' => 'canonical']],
            'declared_contract_source' => [
                ['label' => 'Durable History', 'value' => 'durable_history'],
     ...
     * @return list<array<string, mixed>>
     */
    private static function repairBlockedReasonOptions(): array
    {
        return [
            [
