...
class V2DashboardWorkflowTest extends TestCase
{
    ...
    public function testShowMarksRowOnlyTerminalActivityFallbackAsUnsupported(): void
    {
        ...
        $this->get('/waterline/api/flows/' . $run->id)
            ...
            ->assertJsonPath('actionability.repair_state', 'blocked')
            ->assertJsonPath('actionability.blocked_reason', 'unsupported_history')
            ->assertJsonPath('actionability.diagnostic_only_evidence', true)
            ->assertJsonPath('actionability.actions.repair.allowed', false)
            ->assertJsonPath('actionability.actions.repair.reason', 'unsupported_history')
            ->assertJsonPath('actionability.actions.repair.derived_from', 'repair_state')
    ...
    public function testShowMarksReceivedSignalWithoutWorkflowTaskAsRepairNeeded(): void
    {
        ...
        $this->get('/waterline/api/flows/' . $run->id)
            ...
            ->assertJsonPath('signals.0.current_task_id', null)
            ->assertJsonPath('signals.0.task_ids', [])
            ->assertJsonPath('signals.0.task_missing', true)
            ->assertJsonPath('commands.0.current_task_id', null)
            ->assertJsonPath('commands.0.task_ids', [])
            ->assertJsonPath('commands.0.task_missing', true)
            ->assertJsonPath('tasks.0.type', 'workflow')
        ...
        $this->getJson('/waterline/api/flows/' . $run->id)
            ->assertStatus(200)
            ->assertJsonPath('signals.0.id', $signal->id)
            ->assertJsonPath('signals.0.current_task_id', $task->id)
            ->assertJsonPath('signals.0.current_task_status', 'ready')
            ->assertJsonPath('signals.0.task_transport_state', 'ready')
            ->assertJsonPath('signals.0.task_ids.0', $task->id)
            ->assertJsonPath('signals.0.task_missing', false)
            ->assertJsonPath('commands.0.current_task_id', $task->id)
            ->assertJsonPath('commands.0.current_task_status', 'ready')
            ->assertJsonPath('commands.0.task_transport_state', 'ready')
            ->assertJsonPath('commands.0.task_ids.0', $task->id)
            ->assertJsonPath('commands.0.task_missing', false)
            ->assertJsonPath('tasks.0.id', $task->id)
    ...
    public function testShowKeepsResolvedChildLineageFromTypedHistoryWhenChildRowDrifts(): void
    {
        config()->set('waterline.engine_source', 'v2');

        $parentInstance = WorkflowInstance::create([
            'id' => 'order-detail-child-drift-parent',
            'workflow_class' => 'ParentWorkflowClass',
            'workflow_type' => 'workflow.parent',
            'run_count' => 1,
        ]);
        ...
        $childInstance = WorkflowInstance::create([
            'id' => 'order-detail-child-drift-child',
            'workflow_class' => 'ChildWorkflowClass',
            'workflow_type' => 'workflow.child',
            'run_count' => 1,
        ]);
        ...
        $childCallId = '01JTESTFLOWLINKDRIFT0001';
        WorkflowLink::create([
            'id' => $childCallId,
            'link_type' => 'child_workflow',
            'sequence' => 1,
        ...
        $this->get('/waterline/api/flows/' . $parentRun->id)
            ...
            ->assertJsonPath('continuedWorkflows.0.child_workflow_run_id', $childRun->id)
            ->assertJsonPath('continuedWorkflows.0.workflow_type', $childRun->workflow_type)
            ->assertJsonPath('continuedWorkflows.0.class', $childRun->workflow_class)
            ->assertJsonPath('continuedWorkflows.0.run_number', $childRun->run_number)
            ->assertJsonPath('continuedWorkflows.0.status', 'completed')
            ->assertJsonPath('continuedWorkflows.0.status_bucket', 'completed')
            ->assertJsonPath('continuedWorkflows.0.closed_reason', 'completed');
    }
    ...
    public function testShowKeepsTimerWaitAndTaskMetadataFromTypedHistoryWhenTimerRowIsMissing(): void
    {
        ...
        $this->get('/waterline/api/flows/' . $run->id)
            ...
            ->assertJsonPath('timers_projection_rebuild_reasons', [])
    ...
    public function testShowMarksRepairableCurrentRun(): void
    {
        ...
        $this->get('/waterline/api/flows/' . $instance->id)
            ...
            ->assertJsonPath('can_issue_terminal_commands', true)
            ->assertJsonPath('can_repair', true)
            ->assertJsonPath('actionability.repair_state', 'repairable')
            ->assertJsonPath('actionability.actions.repair.allowed', true)
            ->assertJsonPath('actionability.actions.repair.reason', null)
            ->assertJsonPath('actionability.actions.repair.derived_from', 'repair_state')
            ->assertJsonPath('liveness_state', 'repair_needed')
            ->assertJsonPath('read_only_reason', null)
    ...
    public function testAutomaticWorkerRecoveryRestoresAcceptedUpdateWorkflowTaskWithUpdateTargetDetail(): void
    {
        ...
        $workflow = WorkflowStub::make(TestOperatorCommandWorkflow::class, 'order-update-waterline-watchdog');
        $workflow->start();
    ...
    public function testAutomaticWorkerRecoveryRestoresAcceptedSignalWorkflowTaskWithSignalTargetDetail(): void
    {
        ...
        $workflow = WorkflowStub::make(TestOperatorCommandWorkflow::class, 'order-signal-waterline-watchdog');
        $workflow->start();
    ...
    public function testShowExposesSignalWithStartIntakeContextOnLinkedCommands(): void
    {
        ...
        $this->get('/waterline/api/flows/' . $runId)
            ->assertOk()
            ->assertJsonPath('linked_intakes_scope', 'selected_run')
            ->assertJsonPath('linked_intakes.0.group_id', $result->intakeGroupId())
            ->assertJsonPath('linked_intakes.0.mode', 'signal_with_start')
            ->assertJsonPath('linked_intakes.0.source', 'workflow_commands.context.intake')
            ->assertJsonPath('linked_intakes.0.complete', true)
            ->assertJsonPath('linked_intakes.0.start_command_id', $result->startCommandId())
            ->assertJsonPath('linked_intakes.0.start_command_sequence', $result->startCommandSequence())
            ->assertJsonPath('linked_intakes.0.start_outcome', $result->startOutcome())
            ->assertJsonPath('linked_intakes.0.primary_command_id', $result->commandId())
            ->assertJsonPath('linked_intakes.0.primary_command_sequence', $result->commandSequence())
            ->assertJsonPath('linked_intakes.0.primary_command_type', 'signal')
            ->assertJsonPath('linked_intakes.0.primary_outcome', $result->outcome())
            ->assertJsonPath('linked_intakes.0.commands.0.type', 'start')
            ->assertJsonPath('linked_intakes.0.commands.1.type', 'signal')
            ->assertJsonPath('commands.0.type', 'start')
